...
public class SpezMetrics {
  ...
  // The view has bucket boundaries that will group measure
  // values into histogram buckets.
  private static final View.Name MESSAGE_SIZE_VIEW_NAME = View.Name.create("spez_message_size");
  ...
  public static final View.Name MSG_RECEIVED_VIEW_NAME =
      View.Name.create("spez_msg_received_count");
  public static final View.Name MSG_PUBLISHED_VIEW_NAME =
      View.Name.create("spez_msg_published_count");
  ...
  public void addMessageSize(long size, String tableName) {
    statsRecorder
        .newMeasureMap()
        .put(MESSAGE_SIZE, size)
        .record(SpezTagging.tagForTable(tableName));
  }

  /** log the recorded to stats. */
  public void logStats() {
    for (var view :
        Arrays.asList(MESSAGE_SIZE_VIEW_NAME, MSG_RECEIVED_VIEW_NAME, MSG_PUBLISHED_VIEW_NAME)) {
      log.info(
